...
```
Additional inspected primary bridge: `https://libraries.dal.ca/Fipat/ta2.html` explicitly places individual TA2 terms in the public domain (the publication itself is CC BY-ND). Its linked official collaborator `https://ta2viewer.openanatomy.org/` provides English/Latin term records with TA2 IDs. Public bundle cached `/tmp/atlas-ta2-viewer.js`; use TypeScript AST string-literal parsing and JSON.parse, never execute remote JavaScript. Exact English/Latin mappings and explicit synonyms may bridge model names to NAER rows. Expand Latin abbreviations M./A./V./N./Lig. only as musculus/arteria/vena/nervus/ligamentum. Inspect all resulting candidates for homographs, variant labels and lost specificity before adding. Retain both TA2 ID/Latin and NAER row evidence; do not call these Chinese labels an official TA2 translation.
