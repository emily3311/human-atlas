...
已目视检查上述参考副本 **PDF 第 368 页（印刷页 346）**。该页左上的条目 **06.1063** 清楚列为“**睫状长神经 / long ciliary nerve**”，中英文均是单数整体词形。因此图谱核心 `long ciliary nerve` 的单数对应来自该直接条目；TA2 6206 `long ciliary nerves` / `nervi ciliares longi` 与 NAER row 3994 `Nn. ciliares longi` 的复数记录只作为同一解剖概念的辅助交叉证据，不是将复数自行改成单数的依据。
