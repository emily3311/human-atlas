...
export async function loadWrongCardResources(fetcher: FetchResource, storage: Pick<Storage, 'getItem'>): Promise<{ cards: KnowledgeCard[]; warning: string }> {
  ...
  } catch { return { cards: [], warning: '执医错题题库暂时无法加载，请重新进入记忆卡重试。' }; }
  let explanations;
  let warning = '';
  try {
  ...
  try {
    ...
    return { cards: buildWrongExamCards(questions, progress, explanations), warning };
  } catch { return { cards: [], warning: '执医错题的本机学习记录暂时无法读取。' }; }
}
